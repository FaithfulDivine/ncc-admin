/**
 * Catch-all router for /api/facebook/*
 *
 * Tự động sinh bởi refactor script — gộp 2 endpoint cũ
 * thành 1 Vercel function duy nhất, route theo segment đầu của path.
 *
 * URL pattern không đổi:
 *   /api/facebook/<action>           → routes['action']
 *   /api/facebook/<action>?foo=bar   → routes['action'] với req.query.foo
 *
 * Để thêm endpoint mới: thêm file vào _handlers/<action>.ts rồi đăng ký ở routes.
 */
import type { VercelRequest, VercelResponse } from '@vercel/node'
import handleAdSpend from './_handlers/ad-spend'
import handleStatus from './_handlers/status'

type Handler = (req: VercelRequest, res: VercelResponse) => Promise<unknown>

const routes: Record<string, Handler> = {
  'ad-spend': handleAdSpend,
  'status': handleStatus,
}
